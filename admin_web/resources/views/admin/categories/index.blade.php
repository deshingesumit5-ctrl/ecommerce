...
@section('content')
<div class="space-y-6" x-data="categoryMaster()">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
        <div>
            ...
            <h1 class="text-2xl font-black tracking-tight text-slate-900">Category Master</h1>
            <p class="text-xs text-slate-500 mt-0.5">Manage main product categories, display sequence and catalog thumbnails.</p>
        </div>
    ...
    <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-3">
        <form action="{{ route('admin.categories.index') }}" method="GET" class="w-full flex flex-col md:flex-row items-center gap-3">
            <div class="relative flex-1 w-full">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-400 text-xs">
                    <i class="fa-solid fa-magnifying-glass"></i>
                </div>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search category name..." class="w-full text-xs rounded-xl border border-slate-200 pl-9 pr-3 py-2 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-emerald-500 focus:outline-none">
            </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 text-[11px] font-bold uppercase tracking-wider text-slate-500 border-b border-slate-200">
                        ...
                        <th class="py-3.5 px-4">Sub-Categories</th>
                        <th class="py-3.5 px-4">Total Products</th>
                ...
                <tbody class="divide-y divide-slate-100 text-xs">
                    @forelse($categories as $index => $cat)
                        <tr class="hover:bg-slate-50/80 transition">
                            <td class="py-3.5 px-4 text-slate-400 font-medium">{{ $categories->firstItem() + $index }}</td>
                            <td class="py-3.5 px-4">
                                <img src="{{ $cat->image ?: 'https://images.unsplash.com/photo-1540420773420-3366772f4999?w=100' }}" class="w-10 h-10 rounded-xl object-cover border border-slate-200 shadow-sm">
                            </td>
                            <td class="py-3.5 px-4">
                            ...
                            <td class="py-3.5 px-4">
                                <a href="{{ route('admin.subcategories.index', ['category_id' => $cat->id]) }}" class="px-2 py-1 rounded-lg bg-emerald-50 text-emerald-700 hover:bg-emerald-100 font-bold transition text-[11px]">
                                    {{ $cat->sub_categories_count }} Sub-Categories &rarr;
                                </a>
                            </td>
                            ...
                            <td class="py-3.5 px-4">
                                <form action="{{ route('admin.categories.toggle', $cat->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold {{ $cat->status === 'active' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600' }}">
                                        <span class="w-1.5 h-1.5 rounded-full mr-1 {{ $cat->status === 'active' ? 'bg-emerald-500' : 'bg-slate-400' }}"></span>
                                        {{ ucfirst($cat->status) }}
                                    </button>
                                </form>
                            </td>
                            <td class="py-3.5 px-4 text-right space-x-1">
                                <button type="button" @click="openEditModal({{ json_encode($cat) }})" class="p-1.5 rounded-lg bg-slate-100 hover:bg-emerald-600 hover:text-white text-slate-600 transition">
                    ...
                    @empty
                        <tr>
                            <td colspan="8">
                                <x-empty-state title="No Categories Found" message="Create your product categories to structure the catalog." actionText="Add Category" actionClick="openCreateModal()" />
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div class="md:hidden divide-y divide-slate-100">
            @forelse($categories as $mCat)
                <div class="p-4 flex items-center justify-between gap-3">
                    <div class="flex items-center space-x-3 min-w-0">
                        <img src="{{ $mCat->image ?: 'https://images.unsplash.com/photo-1540420773420-3366772f4999?w=100' }}" class="w-12 h-12 rounded-xl object-cover border border-slate-200 flex-shrink-0">
                        <div class="min-w-0">
                            <span class="font-bold text-slate-900 text-xs block truncate">{{ $mCat->name }}</span>
                            <span class="text-[10px] text-slate-500">{{ $mCat->products_count }} products &bull; Order {{ $mCat->display_order }}</span>
                        </div>
                    </div>
                    <button type="button" @click="openEditModal({{ json_encode($mCat) }})" class="px-3 py-1.5 rounded-lg bg-slate-100 text-xs font-semibold text-slate-700">Edit</button>
                </div>
            @empty
                <div class="p-4"><x-empty-state title="No Categories" actionText="Add Category" actionClick="openCreateModal()" /></div>
            @endforelse
        </div>

        @if($categories->hasPages())
            <div class="p-4 border-t border-slate-100 bg-slate-50">{{ $categories->links() }}</div>
        @endif
            ...
            <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full p-6 border border-slate-200">
                <div class="flex items-center justify-between pb-3 border-b border-slate-100">
                    <h3 class="text-base font-bold text-slate-900 flex items-center space-x-2">
                        ...
                        <span x-text="isEdit ? 'Edit Category' : 'Add New Category'"></span>
                    </h3>
                ...
                <form :action="formAction" method="POST" enctype="multipart/form-data" class="mt-4 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" x-model="formData.name" required placeholder="e.g. Farm Fresh Vegetables" class="w-full text-xs rounded-xl border border-slate-200 px-3 py-2 focus:ring-2 focus:ring-emerald-500 focus:outline-none">
                    </div>

                    <!-- IMAGE UPLOAD & PREVIEW SECTION (REPLACED URL FIELD) -->
                    <div class="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-2">
                        <div class="flex items-center justify-between">
                            <label class="block text-xs font-semibold text-slate-700">Category Photo</label>
                        ...
                        <div x-show="imagePreview" class="relative mt-2 p-2 bg-white rounded-xl border border-slate-200 flex items-center justify-between shadow-sm">
                            <div class="flex items-center space-x-3 overflow-hidden">
                                <img :src="imagePreview" alt="Category Preview" class="w-14 h-14 rounded-lg object-cover border border-slate-200 shadow-sm flex-shrink-0">
                                <div class="min-w-0">
                                    <span class="text-xs font-bold text-slate-800 block truncate" x-text="fileName || 'Category Image'"></span>
                    ...
                    <div class="flex justify-end space-x-2 pt-3 border-t border-slate-100">
                        ...
                        <button type="submit" class="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-xs font-bold text-white shadow-md shadow-emerald-600/20">Save Category</button>
                    </div>
                </form>
            </div>
        </div>
...
<script>
    function categoryMaster() {
        return {
            ...
            fileName: '',
            removeImage: '0',
            formData: { name: '', image: '', display_order: 1, status: 'active' },
            handleFileSelect(e) {
            ...
            openCreateModal() {
                ...
                this.formData = { name: '', image: '', display_order: 1, status: 'active' };
                this.modalOpen = true;
            },
            openEditModal(cat) {
                this.isEdit = true;
                ...
                this.fileName = cat.image ? 'Current Category Image' : '';
                this.removeImage = '0';
                if (this.$refs.imageFileInput) {
                    this.$refs.imageFileInput.value = '';
                }
                this.formData = { name: cat.name, image: cat.image || '', display_order: cat.display_order, status: cat.status };
                this.modalOpen = true;
            }
        };
    }
